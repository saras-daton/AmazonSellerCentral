{% if var('ListFinancialEventsRefundRevenue') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('ListFinancialEventsRefundRevenue', 'ListFinancialEventsRefundRevenue_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('ListFinancialEventsRefundRevenue', 'ListFinancialEventsRefundRevenue_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('ListFinancialEventsRefundRevenue', 'ListFinancialEventsRefundRevenue_temp')}}    
    )
