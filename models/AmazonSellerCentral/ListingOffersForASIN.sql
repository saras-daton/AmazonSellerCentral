{% if var('ListingOffersForASIN') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('ListingOffersForASIN', 'ListingOffersForASIN_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('ListingOffersForASIN', 'ListingOffersForASIN_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('ListingOffersForASIN', 'ListingOffersForASIN_temp')}}    
