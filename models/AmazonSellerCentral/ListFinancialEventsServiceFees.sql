{% if var('ListFinancialEventsServiceFees') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('ListFinancialEventsServiceFees', 'ListFinancialEventsServiceFees_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('ListFinancialEventsServiceFees', 'ListFinancialEventsServiceFees_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('ListFinancialEventsServiceFees', 'ListFinancialEventsServiceFees_temp')}}    
    )
