{% if var('ListFinancialEventsOrderPromotions') %}
    {{ config( enabled = True,
    post_hook = "drop table {{this|replace('ListFinancialEventsOrderPromotions', 'ListFinancialEventsOrderPromotions_temp')}}"
    ) }}
{% else %}
...
{% endif %}
    ...
    {% for i in results_list %}
        ...
        {%- set query -%}
        {{action1}}
        {{tbl|replace('ListFinancialEventsOrderPromotions', 'ListFinancialEventsOrderPromotions_temp')}}
    ...
    {% endfor %}
    select * from {{this|replace('ListFinancialEventsOrderPromotions', 'ListFinancialEventsOrderPromotions_temp')}}    
    )
